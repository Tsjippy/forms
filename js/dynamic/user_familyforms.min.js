var user_family  = new function(){console.log('Dynamic user_family forms js loaded');document.addEventListener('DOMContentLoaded',function(){FormFunctions.tidyMultiInputs();let forms=document.querySelectorAll(`[data-formid="12"]`);forms.forEach(form=>{form.querySelectorAll(`select, input, textarea`).forEach(el=>user_family.p(el));});});var prevEl='';var q=function(event){var el=event.target;form=el.closest('form');var n=el.getAttribute('name');if(n==''||n==undefined){if(el.closest('.nice-select-dropdown')!=null&&el.closest('.inputwrapper')!=null){el.closest('.inputwrapper').querySelectorAll('select').forEach(select=>{if(el.dataset.value==select.value){el=select;n=select.name;}});}else{return;}}if(el==prevEl){return;}prevEl=el;setTimeout(function(){prevEl='';},50);if(n=='nextBtn'){FormFunctions.nextPrev(1);}else if(n=='prevBtn'){FormFunctions.nextPrev(-1);}user_family.p(el);};window.addEventListener('click',q);window.addEventListener('input',q);this.p=function(el){var n=el.getAttribute('name');var form=el.closest('form');if(n=='family[partner]'){var v_1=FormFunctions.getFieldValue('family[partner]',form,true,'',true);if(v_1!=''){form.querySelectorAll(`[name="married_since_label"], [name="family[weddingdate]"], [name="family[picture]_files[]"], [name^="family[children]["]`).forEach(el=>{if(!el.closest('.inputwrapper').matches('.action-processed')){el.closest('.inputwrapper').classList.add('action-processed');FormFunctions.changeVisibility('remove',el,user_family.p);}});document.querySelectorAll('.action-processed').forEach(el=>{el.classList.remove('action-processed')});}if(v_1==''){form.querySelectorAll(`[name="married_since_label"], [name="family[weddingdate]"], [name="family[picture]_files[]"], [name^="family[children]["]`).forEach(el=>{if(!el.closest('.inputwrapper').matches('.action-processed')){el.closest('.inputwrapper').classList.add('action-processed');FormFunctions.changeVisibility('add',el,user_family.p);}});document.querySelectorAll('.action-processed').forEach(el=>{el.classList.remove('action-processed')});}}};};if(typeof(urlSearchParams)=='undefined'){window.urlSearchParams=new URLSearchParams(window.location.search.replaceAll('&amp;','&'));}Array.from(urlSearchParams).forEach(array=>document.querySelectorAll(`[name^='${array[0]}']`).forEach(el=>FormFunctions.changeFieldValue(el,array[1],user_family.p,el.closest('form'),)));
