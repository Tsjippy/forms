var user_family = new function(){
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		let forms = document.querySelectorAll(`[data-formid="12"]`);
		forms.forEach(form=>{
		...
		if(elName == 'family[partner]'){
			...
			if(value_1 != ''){
				form.querySelectorAll(`[name="married_since_label"], [name="family[weddingdate]"], [name="family[picture]_files[]"], [name^="family[children]["]`).forEach(el=>{
						...
						if(!el.closest('.inputwrapper').matches('.action-processed')){
							...
							FormFunctions.changeVisibility('remove', el, user_family.processFields);
						}
				});
			...
			if(value_1 == ''){
				form.querySelectorAll(`[name="married_since_label"], [name="family[weddingdate]"], [name="family[picture]_files[]"], [name^="family[children]["]`).forEach(el=>{
						...
						if(!el.closest('.inputwrapper').matches('.action-processed')){
							el.closest('.inputwrapper').classList.add('action-processed');
							FormFunctions.changeVisibility('add', el, user_family.processFields);
						}
				});
				document.querySelectorAll('.action-processed').forEach(el=>{el.classList.remove('action-processed')});
			}
...
Array.from(urlSearchParams).forEach(array => document.querySelectorAll(`[name^='${array[0]}']`).forEach(el => FormFunctions.changeFieldValue(el, array[1], user_family.processFields, el.closest('form'), )));
